//  Reads the values from global tasks[], totalTasks, availableHours set by
//  main.cpp and makes prediction using two models
// i.e, descision tree and k's nearest neighbours (descision tree is primary
// prediction) and labels scenarios
// as 0, 1 and 2 indicating the recommended usage of sorting, greedy and dynamic
// programming strategy respectively
...
ScenarioFeatures extractFeatures() {
  ...
  // finding the tasks with minimum and maximum importance, and also the number
  // of urgent tasks
  ...
  for (int i = 0; i < totalTasks; i++) {
    ...
    if (tasks[i].importance < minImp)
      minImp = tasks[i].importance;
    if (tasks[i].importance > maxImp)
      maxImp = tasks[i].importance;
    if (tasks[i].deadline <= 2)
      urgentCount++;
  }
...
//  Training Dataset using 15 created scenarios covering 3 different scenario
//  types labelled 0 if low time ratio and low urgency, 1 if moderate time ratio
//  and high urgency, 2 if high ratio and moderate urgency
...
// 15 training scenarios consisting of different time availability and urgency
// levels
const vector<TrainingExample> TRAINING_DATA = {
    {1.2, 3.8, 0, 2.0, 0}, {1.4, 4.0, 1, 2.0, 0}, {1.3, 3.5, 0, 3.0, 0},
    {1.5, 4.2, 1, 2.5, 0}, {2.2, 4.3, 6, 2.0, 1}, {2.0, 4.0, 4, 2.0, 1},
    {2.3, 4.5, 5, 1.5, 1}, {1.8, 3.8, 4, 2.0, 1}, {2.3, 4.0, 5, 2.5, 1},
    {3.3, 4.0, 4, 3.0, 2}, {2.8, 4.5, 3, 3.0, 2}, {3.5, 4.2, 3, 3.5, 2},
...
// using euclidean distance formula to caluclate the similarity of training data
// and given data
...
void printTrainingData() {
  ...
  cout << left << setw(6) << "No." << setw(20) << "TimePressureRatio"
       << setw(18) << "AvgImportance" << setw(14) << "UrgentTasks" << setw(22)
       << "ImportanceVariation" << "Label\n";
  cout << string(88, '-') << "\n";
  for (int i = 0; i < (int)TRAINING_DATA.size(); i++) {
    const auto &ex = TRAINING_DATA[i];
    cout << setw(6) << i + 1 << setw(20) << fixed << setprecision(1)
         << ex.timePressureRatio << setw(18) << ex.avgImportance << setw(14)
         << ex.urgentTasks << setw(22) << ex.importanceVariation
         << strategyName(ex.label) << "\n";
  }
}
